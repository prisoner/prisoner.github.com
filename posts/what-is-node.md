...
![http://img03.taobaocdn.com/tps/i3/T15q4UXa4CXXXXXXXX-320-419.jpg][1]

## Node不是万能药！但的确能解决一些关键问题。

学习Node不是一件轻松事儿，但你所收到的回报是对得起你的付出的。因为当下Web应用开发中的诸多难题唯有JavaScript才能解决。

作者：[Brett McLaughlin][2] ，翻译：[拔赤][3]
...
感谢Mike Amundsen，他给出了这段代码的相似的实现。这个例子是由Devon Govett在Nettuts+上提交的一段代码，尽管已经根据新版本的Node作了更新，但Devon的[整个帖子][5]是一个非常好的入门学习教材，对于初学者来说更是如此。

如果你是一个新手，你可以将上述代码保存到一个文本文件中，命名为NodeFileServer.js。在运行之前你需要一个Node运行环境，最新的Node版本可以从[官网][6]下载[这个文件][7]或者从[github上][8]将源码取下来。你需要编译源码，如果你没有用过Unix、对make和configure不甚熟悉，则需要查阅在[线编译手册][9]来寻求帮助。
...
首先调用了函数require()，require()是程序员最常用的函数之一。实际上，在[CommonJS规范][13]中也有提到这个函数，在[讨论到关于JavaScript模块][14]概念的时候有提及，此外，Davd Flanagan在2009年的[一个很酷的实现][15]中也有提到。换句话说，require()对于你来说可能是个新鲜事物，但它不是Node随意添加的一个函数，他是使用JavaScript进行模块化编程的核心概念，Node将这一特性发挥的淋漓尽致。
...
> 或许有些人对上文提到的问题不以为然，因为你知道有很多方法来规避直接eval()带来的问题，你会使用诸如[JSON.parse()][18]来代替eval()。同样有很多[令人信服的论据][19]鼓励我们小心的使用eval()。这些东东都是值得进一步讨论的。但不管怎样，看一看eval()带来了太多类似[栈溢出（Stack Overflow）][20]这类的问题吧，你会发现大部分程序员并未正确或者安全的使用eval()。这着实是一个问题。因为太多菜鸟程序员似乎根本没有意识到eval()的问题有多严重。
...
此外，Node也计划实现“进程分支”（process forking），[HTML5 Web Workers API][21]为更复杂的进程控制提供了引擎（规范）支持。同样，如果你采用基于事件的模型来架构web应用，你的程序可能至少有100多个场景需要线程的支持。最终你会发现，你的编程思路和思考问题的方式发生了改变，你的注意力将放在服务器端处理请求的逻辑上，而不必在乎Node如何工作。
...
学习Node所面临的挑战是，你需要更加活跃思维，将程序拆成低耦合的小片段，然后像组装数组一样的组装他们。但Node和基于事件的I/O并不能解决所有问题，但确定的是，很多关键问题，只能依靠Node来解决。
  ...
  [1]: http://img03.taobaocdn.com/tps/i3/T15q4UXa4CXXXXXXXX-320-419.jpg
  [2]: http://www.oreillynet.com/pub/au/152
  [3]: http://jayli.github.com
  [4]: http://oreilly.com/catalog/0636920021506
  [5]: http://net.tutsplus.com/tutorials/javascript-ajax/learning-serverside-javascript-with-Node-js/
  [6]: http://Nodejs.org/
  [7]: http://Nodejs.org/docs/latest/#download
  [8]: https://github.com/joyent/Node
  [9]: https://github.com/joyent/Node/wiki/Installation
  [10]: http://img04.taobaocdn.com/tps/i4/T1rtxUXjNzXXXXXXXX-500-335.png
  [11]: http://jayli.github.com/whatisnode/assets/interacting.png
  [12]: http://jayli.github.com/whatisnode/assets/mockup.png
  [13]: http://www.commonjs.org
  [14]: http://wiki.commonjs.org/wiki/Modules/1.1
  [15]: http://www.davidflanagan.com/2009/11/commonjs-module.html
  [16]: http://img03.taobaocdn.com/tps/i3/T1oR5lXnhoXXXXXXXX-500-292.jpg
  [17]: http://stackoverflow.com/questions/86513/why-is-using-javascript-eval-function-a-bad-idea
  [18]: http://stackoverflow.com/questions/1843343/json-parse-vs-eval
  [19]: http://javascriptweblog.wordpress.com/2010/04/19/how-evil-is-eval/
  [20]: http://stackoverflow.com/
  [21]: http://www.whatwg.org/specs/web-apps/current-work/complete/workers.html
